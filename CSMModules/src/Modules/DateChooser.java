...
public class DateChooser extends JPanel {
    ...
    public DateChooser(JFrame jform, JTextField jtext, String format) {
        this(new Date());
        this.jform = jform;
        this.jtext = jtext;
        this.format = format;
    ...
    private void commit() {
        date_c = sdf.format(select.getTime());
        date_u = date_c.substring(0, 4) + "-" + date_c.substring(5, 7) + "-" + date_c.substring(8, 10);
        System.out.println("选中的日期是：" + getDate("") + ":" + getDate("yyyy-mm-dd"));
        // showDate.setText(date_c);
        showDate.setText(getDate("中文格式"));
        if (jtext != null) {
                jtext.setText(getDate(format));
        }
        hidePanel();
        if (jform != null) {
            this.jform.dispose();
    ...
    private class LabelManager {
        ...
        public void setSelect(Point p, boolean b) {
            //如果匙拖动,则要优化一下,以提高效率
            if (b) {
                //表示匙否能返回,不用比较完所有de标签,能返回de标志就匙把上一个标签和
                //将要显示de标签找到了就可以了
